...
from .pthdataset import PthDataset
...
class PthDataModule(HAMERDataModule):
    def __init__(self, cfg: CfgNode, dataset_cfg: CfgNode) -> None:
        super().__init__(cfg, dataset_cfg)

    def setup(self, stage):
        if self.train_dataset == None:
            self.train_dataset = PthDataset(
                data_dir=self.dataset_cfg["data_dir"][0]["data_dir"]
            )

            self.val_dataset = PthDataset(
                data_dir=self.dataset_cfg["data_dir"][0]["data_dir"]
            )
            self.mocap_dataset = PthDataset(
                data_dir=self.dataset_cfg["data_dir"][0]["data_dir"]
            )
